var app = new Vue({
	...
	computed: {
	  ...
	  loadingPercentage: function(){
		console.log()
		var self = this
		if (this.loadingStartedTime && this.estimatedLoadingTime){
			var diff = Math.floor((new Date() - this.loadingStartedTime )/ 10)
			diff = diff / this.estimatedLoadingTime
	...
	methods: {
	  startNav: function(){
		  ...
		  this.estimatedLoadingTime = 40
		  this.loadingStartedTime = new Date()
  ...
  function updateRunInfo (self){
	$.get("/navigationStatus", function(data){
	  ...
	  if (self.mode == self.mode_enum.loading){
		if(!data.loading){
		  self.mode = self.mode_enum[data.state]
		  self.estimatedLoadingTime = null
		  self.loadingStartedTime = null
		}
	  }
